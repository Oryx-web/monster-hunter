  ...
  return (
    <div className="space-y-2 max-h-screen overflow-y-auto bg-no-repeat bg-cover" style={{ backgroundImage: `url('${import.meta.env.BASE_URL}background.webp')`}}>
      {monsters.map((monster) => (
        ...
        >
          <img className="w-24 lg:w-30 xl:w-36" src={getMonsterIcon(monster.name)} alt={monster.name} loading='lazy'/>
          <p className="text-lg lg:text-xl xl:text-2xl font-semibold text-white text-outline">{monster.name}</p>
        </div>
      ))}
    </div>
  );
