...
export default function MonsterDetails({ selectedMonster, getMonsterIcon, getWeaknessIcon, status }) {
  if (!selectedMonster) {
    return <h2 className="text-gray-700">Loading...</h2>;
  }
  ...
  return (
    <>
      ...
      <div className="mt-4 lg:w-3/4 border border-[#7B4F2D] rounded-md">
      ...
      <div className="mt-4 border ml-5 mr-5 w-2/3 sm:w-full border-[#7B4F2D] rounded-md">
        <p className="text-2xl text-yellow-300 font-serif text-center text-outline bg-[#534a4a67]">Weaknesses</p>
        <ul className="px-2 xl:px-3 py-3 grid grid-cols-1 lg:grid-cols-2 xl:grid-cols-3 gap-5 justify-between justify-items-center text-[#54361E]">
            ...
            !weakness.condition ? (
              <li key={index} className="flex items-center">
                <p className="flex items-center">
                  {weakness.element}(
                  <img
                    className="w-8 sm:w-6 lg:w-8 xl:w-10"
