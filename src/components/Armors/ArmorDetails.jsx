...
export default function ArmorDetails({ selectedArmor, getArmorIcon }) {
  ...
  return (
    <>
            ...
            {selectedArmor.pieces.map((piece) => (
              piece.assets !== null ? (
                piece.assets.imageMale !== null ? (
                  ...
                  <p key={piece.id} className="w-40 h-20 flex items-center justify-center justify-items-center text-white text-center">
                    No Armor Piece Available
                  </p>
                )
              ) : (
                <p key={piece.id} className="w-40 h-20 flex items-center justify-center justify-items-center text-white text-center">
      ...
      {/* Armor Parts */}
      <div className={`mt-4 border ml-5 mr-5 ${selectedArmor.pieces.length === 1 ? "w-2/4" : "w-1/1"} border-gray-700 rounded-md`}>
        <p className="text-2xl text-white font-serif text-center text-outline bg-[#534a4a67]">Parts</p>
        <ul className={`px-3 py-3 grid grid-cols-1 ${selectedArmor.pieces.length === 1 ? "sm:grid-cols-1" : "sm:grid-cols-3"} gap-5 justify-between justify-items-center text-white`}>
          {selectedArmor.pieces.map((piece) => (
            <li key={piece.id} className="flex items-center">
              <p className="flex items-center">
                <img className="w-10 h-10" src={getArmorIcon(piece.type)} alt={piece.type} loading="lazy" /> {piece.name}
              </p>
            </li>
          ))}
        </ul>
